...
# Creates the shared app login role ($WEB_DB_USER) and the databases it owns.
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_USER" <<-EOSQL
  DO \$\$
  BEGIN
    IF NOT EXISTS (SELECT FROM pg_roles WHERE rolname = '${WEB_DB_USER}') THEN
      CREATE ROLE "${WEB_DB_USER}" LOGIN PASSWORD '${WEB_DB_PASSWORD}';
    END IF;
  END
  \$\$;
EOSQL
...
for db in authentication chat money_manager recipes_book notes; do
  if ! psql -tA --username "$POSTGRES_USER" --dbname "$POSTGRES_USER" \
    ...
    psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_USER" \
      -c "CREATE DATABASE $db OWNER \"${WEB_DB_USER}\""
