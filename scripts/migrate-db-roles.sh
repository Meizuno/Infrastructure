...
#
# Reads the per-app passwords from the environment (same names as .env). Run
# from the stack dir AFTER `git pull` but BEFORE redeploying the apps:
#
#   set -a; . ./.env; set +a; ./scripts/migrate-db-roles.sh
#
...
SU="${POSTGRES_SUPERUSER:-admin}"
psql() { docker compose exec -T postgres psql -v ON_ERROR_STOP=1 -U "$SU" "$@"; }

: "${AUTH_DB_PASSWORD:?set AUTH_DB_PASSWORD (source ./.env first)}"
: "${MONEY_DB_PASSWORD:?set MONEY_DB_PASSWORD}"
: "${RECIPES_DB_PASSWORD:?set RECIPES_DB_PASSWORD}"
: "${NOTES_DB_PASSWORD:?set NOTES_DB_PASSWORD}"
